...
class MiscCog(commands.Cog):
    ...
    async def replace_roles_cmd(
        ...
        """Replaces roles generated by "Generate Rolelist Roles"."""

        if delete_original and not await utils.mod_check(interaction):
            raise utils.SDGException('You aren\'t allowed to delete original message without being trusted.')

        if not roles_message.content:
            raise utils.SDGException('Message has no content')
